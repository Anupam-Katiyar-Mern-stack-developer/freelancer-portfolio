import { motion } from "motion/react";

import useSiteData from "../../hooks/useSiteData";

import { iconMap } from "../../utils/iconMap";

import SectionTitle from "../common/SectionTitle";

const WhatIDoSection = () => {
  const { siteData } = useSiteData();

  const work = siteData?.aboutPage?.work;

  if (!work) {
    return null;
  }

  return (
    <section className="relative overflow-hidden bg-[#F8FAFC] px-5 py-20 sm:px-8 lg:px-10 lg:py-28">

      <div className="mx-auto max-w-7xl">

        <SectionTitle
          eyebrow={work.eyebrow}
          title={work.heading}
          highlight={work.highlight}
        />

        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">

          {work.items.map((item, index) => {
            const Icon = iconMap[item.icon];

            return (
              <motion.article
                key={item.title}
                initial={{
                  opacity: 0,
                  y: 30,
                }}
                whileInView={{
                  opacity: 1,
                  y: 0,
                }}
                viewport={{
                  once: true,
                }}
                transition={{
                  delay: index * 0.08,
                }}
                whileHover={{
                  y: -8,
                }}
                className="group rounded-[26px] border border-slate-200 bg-white p-6 shadow-[0_15px_50px_rgba(15,23,42,0.04)] transition hover:border-blue-200 hover:shadow-[0_30px_80px_rgba(37,99,235,0.10)]"
              >

                <div className="flex h-13 w-13 items-center justify-center rounded-2xl bg-slate-950 text-xl text-white transition group-hover:bg-blue-600">
                  {Icon && <Icon />}
                </div>

                <h3 className="mt-6 font-['Space_Grotesk'] text-xl font-bold text-slate-950">
                  {item.title}
                </h3>

                <p className="mt-3 font-[Manrope] text-sm leading-7 text-slate-500">
                  {item.description}
                </p>

              </motion.article>
            );
          })}

        </div>

      </div>
    </section>
  );
};

export default WhatIDoSection;